...
`tar_stan_mcmc_rep_draws()` creates targets to run MCMC multiple times
per model and save only the draws from each run.
...
`tar_stan_mcmc_rep_draws()` returns a list of target objects. See the
...
objects returned by
`tar_stan_mcmc_rep_draws(name = x, stan_files = "y.stan")` are as
follows.
...
- `x_y`: dynamic branching target to run MCMC once per dataset. Each
  dynamic branch returns a tidy data frames of draws corresponding to a
  batch of Stan data from `x_data`.
