...
`tar_stan_mle_rep_draws()` creates targets to run maximum likelihood
multiple times per model and save the MLEs in a wide-form draws-like
data frame.
...
`tar_stan_mle_rep_draws()` returns a list of target objects. See the
...
[`tar_stan_mle_rep_summary()`](https://docs.ropensci.org/stantargets/reference/tar_stan_mle_rep_summary.md)
