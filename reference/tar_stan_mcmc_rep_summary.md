...
Targets to run MCMC multiple times and save only the summary output from
each run.
...
`tar_stan_mcmc_rep_summary()` returns a list of target objects. See the
...
objects returned by
`tar_stan_mcmc_rep_summary(name = x, stan_files = "y.stan")` are as
follows.
...
- `x_y`: dynamic branching target to run MCMC once per dataset. Each
  dynamic branch returns a tidy data frames of summaries. corresponding
  to a batch of Stan data from `x_data`.
