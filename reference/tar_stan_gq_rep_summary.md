...
`tar_stan_gq_rep_summaries()` creates targets to run generated
quantities multiple times and save only the summaries from each run.
...
`tar_stan_gq_rep_summaries()` returns a list of target objects. See the
...
objects returned by
`tar_stan_gq_rep_summary(name = x, stan_files = "y.stan")` returns a
