...
Targets to run a Stan model once with variational Bayes and save
multiple outputs.
...
`tar_stan_vb()` returns a list of target objects. See the "Target
...
objects returned by `tar_stan_vb(name = x, stan_files = "y.stan", ...)`
are as follows.
...
- `x_vb_y`: run variational Bayes on the model and the dataset. Returns
  a `cmdstanr` `CmdStanVB` object with all the results.

- `x_draws_y`: extract draws from `x_vb_y`. Omitted if `draws = FALSE`.
  Returns a tidy data frame of draws.

- `x_summary_y`: extract compact summaries from `x_vb_y`. Returns a tidy
  data frame of summaries. Omitted if `summary = FALSE`.

## Details

Most of the arguments are passed to the `$compile()`, `$variational()`,
and `$summary()` methods of the `CmdStanModel` class. If you previously
