...
`tar_stan_vb_rep_summaries()` creates targets to run variational Bayes
multiple times and save only the summary output from each run.
...
objects returned by
`tar_stan_vb_rep_summary(name = x, stan_files = "y.stan")` are as
follows.
...
- `x_y`: dynamic branching target to run variational Bayes once per
  dataset. Each dynamic branch returns a tidy data frames of summaries
...
[`tar_stan_vb_rep_draws()`](https://docs.ropensci.org/stantargets/reference/tar_stan_vb_rep_draws.md)
